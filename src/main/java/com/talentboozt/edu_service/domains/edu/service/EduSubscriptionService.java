...
import com.talentboozt.edu_service.domains.edu.application.service.SubscriptionRenewalApplicationService;
...
public class EduSubscriptionService {
    ...
    @Autowired
    private SsoGatewayPort ssoGateway;

    @Autowired
    private com.talentboozt.edu_service.domains.edu.repository.mongodb.EWorkspacesRepository workspacesRepository;
    ...
    public void syncUserRoles(String userId, ESubscriptionPlan newPlan) {
        userRepository.findById(userId).ifPresent(user -> {
            ...
            // Synchronize with global SSO identity via ACL
            try {
                ssoGateway.syncRoles(userId, roles.stream().map(Enum::name).collect(java.util.stream.Collectors.toList()));
            } catch (Exception e) {
    ...
    public void provisionManualEnterprise(String userId, Double commissionRate, Integer maxCourses, Integer maxMembers, String notes) {
        ...
        try {
            java.util.List<com.talentboozt.edu_service.domains.edu.model.EWorkspaces> workspaces = workspacesRepository.findByOwnerId(userId);
            if (workspaces != null) {
                for (com.talentboozt.edu_service.domains.edu.model.EWorkspaces ws : workspaces) {
